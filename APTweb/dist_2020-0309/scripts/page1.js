...
function getStatustableData(html){
    ...
    var paddingkey;
    Array.prototype.forEach.call($body,function(element,index){
        console.log(element);

        //var element = $body[key];
        if($(element).hasClass('padding')){
            paddingkey = index;
        }
    });
    ...
    var $table;
    var $footer;
    $header = $body.slice(0, paddingkey);
    $table = $body[paddingkey];
    $footer = $body.slice(paddingkey+1, $body.length-2);
    // 设置header和footer
    seHeaderAndFooter( $header, $footer );
 ...
 */
function seHeaderAndFooter( $header, $footer ){
...
function getCategory( $table ){
    ...
    request.done(function( category ) {
        ...
        // 初始化汇总table
        initSummaryTable( $table, categoryData);
 ...
 * @param {object} outDoc 
 */
function initSummaryTable($table, categoryData){
    var id = 1;
    var oldColumns = [];
    ...
    var $tableth = $table.find('tbody  th');
                ...
                if (subArray.hasOwnProperty(subkey)) {
                    ...
                    newData.forEach(function (record) {
                        ...
                        if( status.includes('FAIL') ) {
                             ...
                             * todos 2020/3/9
                             */
                            statusHref = status.split('"')[1];
                            oldhref = linkFormatter(statusHref);
